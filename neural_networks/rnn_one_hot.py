...
class RNNOneHot(rnn.RNNBase):
    ...
    def _get_model_filename(self, epochs):
        ...
        '''
        filename = "rnn_cce_db" + str(self.diversity_bias) + "_r" + str(
            self.regularization) + "_" + self._common_filename(epochs)
        return filename
    ...
    def _prepare_networks(self, n_items):
        ...
        if not self.use_movies_features:
            l_recurrent = self.recurrent_layer(self.l_in, self.l_mask,
                                               true_input_size=self.n_items + self._n_optional_features(),
        ...
        self.l_out = lasagne.layers.DenseLayer(l_last_slice, num_units=self.n_items,
