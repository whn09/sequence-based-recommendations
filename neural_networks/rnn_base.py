...
class RNNBase(object):
    ...
    def _compile_test_function(self):
        ...
        theano_test_function = theano.function(self.theano_inputs, deterministic_output, allow_input_downcast=True,
    ...
    def _gen_mini_batch(self, sequence_generator, test=False, max_reuse_sequence=np.inf):
        ...
        while True:
            ...
            while j < batch_size:
                ...
                if not test:
                    seq_lengths = sorted(random.sample(xrange(2, len(sequence)),
                                                       min([batch_size - j, len(sequence) - 2, max_reuse_sequence])))
                else:
    ...
    def load(self, filename):
        ...
        f.close()
        # print('param:', len(param), param[0].shape)
    ...
    def _get_optional_features(self, item, user_id):
        return np.concatenate(
            (self._get_ratings_features(item), self._get_movies_features(item), self._get_user_features(user_id)))
