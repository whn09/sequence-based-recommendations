...
class FISM(MFBase):
    ...
    def __init__(self, k=100, alpha=0.5, loss="auc", **kwargs):
    ...
    def _get_model_filename(self, epochs):
        ...
        '''
        filename = "fism_" + self.loss + "_ne" + str(epochs) + "_lr" + str(self.init_learning_rate) + "_an" + str(
            self.annealing_rate) + "_k" + str(self.k) + "_reg" + str(self.reg) + "_ini" + str(self.init_sigma)
    ...
    def item_score(self, user_items, item=None):
        ...
        '''
        if item is not None:
            return self.bias[item] + np.power(len(user_items), -self.alpha) * np.dot(self.V[user_items, :].sum(axis=0),
                                                                                     self.H[item, :])
        else:
            return self.bias + np.power(len(user_items), -self.alpha) * np.dot(self.V[user_items, :].sum(axis=0),
                                                                               self.H.T)
    ...
    def auc_sgd_step(self, user_items, true_item, false_item):
        ...
        self.V[user_items, :] += self.learning_rate * (delta * np.power(len(user_items), -self.alpha) * (
                    self.H[true_item, :] - self.H[false_item, :]) - self.reg * self.V[user_items, :])
        self.H[true_item, :] += self.learning_rate * (
                    delta * np.power(len(user_items), -self.alpha) * V_sum - self.reg * self.H[true_item, :])
        self.H[false_item, :] += self.learning_rate * (
                    -delta * np.power(len(user_items), -self.alpha) * V_sum - self.reg * self.H[false_item, :])
    ...
    def rmse_sgd_step(self, user_items, item, rating):
        ...
        self.V[user_items, :] += self.learning_rate * (
                    delta * np.power(len(user_items), -self.alpha) * self.H[item, :] - self.reg * self.V[user_items, :])
        self.H[item, :] += self.learning_rate * (
                    delta * np.power(len(user_items), -self.alpha) * V_sum - self.reg * self.H[item, :])
