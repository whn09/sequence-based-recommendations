...
class BPRMF(MFBase):
    ...
    def _get_model_filename(self, epochs):
        ...
        '''
        filename = "bprmf_ne" + str(epochs) + "_lr" + str(self.init_learning_rate) + "_an" + str(
    ...
    def sgd_step(self, user, true_item, false_item):
        ...
        # Update CF
        V_mem = self.V[user, :]
        self.V[user, :] += self.learning_rate * (
                    delta * (self.H[true_item, :] - self.H[false_item, :]) - self.reg * self.V[user, :])
