...
class Fossil(MFBase):
    ...
    def __init__(self, k=32, order=1, alpha=0.2, **kwargs):
    ...
    def item_score(self, user_id, user_items, item=None):
        ...
        if user_id is None:
            short_term = np.dot((self.eta_bias + self.eta.mean(axis=0))[:effective_order],
                                self.V[user_items[:-effective_order - 1:-1], :])
        else:
            short_term = np.dot((self.eta_bias + self.eta[user_id, :])[:effective_order],
                                self.V[user_items[:-effective_order - 1:-1], :])
    ...
    def sgd_step(self, user_id, user_items, false_item):
        ...
        # Compute Update
        V_update = self.learning_rate * (delta * np.power(len(user_items), -self.alpha) * (
                    self.H[true_item, :] - self.H[false_item, :]) - self.reg * self.V[user_items, :])
        V_update2 = self.learning_rate * delta * np.outer((self.eta_bias + self.eta[user_id, :])[:effective_order],
                                                          self.H[true_item, :] - self.H[false_item, :])
        ...
        eta_bias_up = self.learning_rate * (delta * np.dot(self.V[user_items[:-effective_order - 1:-1], :],
                                                           self.H[true_item, :] - self.H[false_item,
                                                                                  :]) - self.reg * self.eta_bias[
                                                                                                   :effective_order])
        eta_up = self.learning_rate * (delta * np.dot(self.V[user_items[:-effective_order - 1:-1], :],
                                                      self.H[true_item, :] - self.H[false_item,
                                                                             :]) - self.reg * self.eta[user_id,
                                                                                              :effective_order])
