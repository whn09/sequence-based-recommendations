...
class FPMC(MFBase):
    ...
    def __init__(self, k_cf=32, k_mc=32, adaptive_sampling=True, sampling_bias=500, **kwargs):
    ...
    def _get_model_filename(self, epochs):
        ...
        '''
        filename = "fpmc_ne" + str(epochs) + "_lr" + str(self.init_learning_rate) + "_an" + str(
            self.annealing_rate) + "_kcf" + str(self.k_cf) + "_kmc" + str(self.k_mc) + "_reg" + str(
    ...
    def sgd_step(self, user, prev_item, true_next, false_next):
        ...
        # Compute error
        x_true = np.dot(self.V_user_item[user, :], self.V_item_user[true_next, :]) + np.dot(
            self.V_prev_next[prev_item, :], self.V_next_prev[true_next, :])
        x_false = np.dot(self.V_user_item[user, :], self.V_item_user[false_next, :]) + np.dot(
            self.V_prev_next[prev_item, :], self.V_next_prev[false_next, :])
        delta = 1 - 1 / (1 + math.exp(
            min(10, max(-10, x_false - x_true))))  # Bound x_true - x_false in [-10, 10] to avoid overflow

        # Update CF
        V_user_item_mem = self.V_user_item[user, :]
        self.V_user_item[user, :] += self.learning_rate * (delta * (
                    self.V_item_user[true_next, :] - self.V_item_user[false_next, :]) - self.reg * self.V_user_item[
                                                                                                   user, :])
        self.V_item_user[true_next, :] += self.learning_rate * (
                    delta * V_user_item_mem - self.reg * self.V_item_user[true_next, :])
        self.V_item_user[false_next, :] += self.learning_rate * (
                    -delta * V_user_item_mem - self.reg * self.V_item_user[false_next, :])

        # Update MC
        V_prev_next_mem = self.V_prev_next[prev_item, :]
        self.V_prev_next[prev_item, :] += self.learning_rate * (delta * (
                    self.V_next_prev[true_next, :] - self.V_next_prev[false_next, :]) - self.reg * self.V_prev_next[
                                                                                                   prev_item, :])
        self.V_next_prev[true_next, :] += self.learning_rate * (
                    delta * V_prev_next_mem - self.reg * self.V_next_prev[true_next, :])
        self.V_next_prev[false_next, :] += self.learning_rate * (
                    -delta * V_prev_next_mem - self.reg * self.V_next_prev[false_next, :])
    ...
    def get_training_sample(self):
        ...
        if self.adaptive_sampling:
            while True:
                ...
                factor_prob = np.abs(
                    np.concatenate((self.V_user_item[user_id, :], self.V_prev_next[prev_item, :]))) * self.var
    ...
    def top_k_recommendations(self, sequence, user_id=None, k=10, exclude=None):
        ...
        output = np.dot(self.V_user_item[user_id, :], self.V_item_user.T) + np.dot(self.V_prev_next[last_item, :],
                                                                                   self.V_next_prev.T)
    ...
    def save(self, filename):
        ...
        np.savez(filename, V_user_item=self.V_user_item, V_item_user=self.V_item_user, V_prev_next=self.V_prev_next,
                 V_next_prev=self.V_next_prev)
