...
def get_file_name(predictor, args):
    return args.dir + re.sub('_ml' + str(args.max_length), '_ml' + str(args.training_max_length),
...
def test_command_parser(parser):
    ...
    parser.add_argument('--metrics', help='List of metrics to compute, comma separated',
                        default='sps,recall,item_coverage,user_coverage,blockbuster_share', type=str)
    ...
    parser.add_argument('--save_rank', help='Save the full comparison of goal and prediction ranking.',
                        action='store_true')
    ...
    if args.number_of_batches == "*" and args.method != "UKNN" and args.method != "MM" and args.method != "POP":
        ...
        for i, f in enumerate(file):
            if batches[i] > last_tested_batch:
                evaluator = run_tests(predictor, f, dataset, args, get_full_recommendation_list=args.save_rank,
                                      k=args.nb_of_predictions)
                print('-------------------')
                print('(', i + 1, '/', len(file), ') results on ' + f)
                print_results(evaluator, args.metrics.split(','), plot=False, file=output_file, n_batches=batches[i],
